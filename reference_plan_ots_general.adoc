...
RAID 类型:: 在 ESXi 上使用直连存储（ DAS ）时，您应确定是使用本地硬件 RAID 控制器还是使用 ONTAP Select 附带的软件 RAID 功能。如果您使用软件RAID、请参见link:reference_plan_ots_storage.html["存储和 RAID 注意事项"]以了解更多信息。
